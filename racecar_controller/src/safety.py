...
class Safety():
    def __init__(self):
        ...
        self.sub = rospy.Subscriber(
            "/scan", LaserScan, self.lidarCB, queue_size=1)
        # /vesc/low_level/ackermann_cmd_mux/input/safety
        self.pub = rospy.Publisher(
            "/ackermann_cmd_mux/input/teleop",  AckermannDriveStamped, queue_size=1)
        self.failsafe_pub = rospy.Publisher('failsafe', String, queue_size=10)
    ...
    def lidarCB(self, msg):
        ...
        if not self.received_data:
            ...
            self.large_bins = np.array(
                [-float('inf'), -1*FAN_ANGLE, FAN_ANGLE, float('inf')])
        ...
        angles = self.angles[(values > msg.range_min) &
